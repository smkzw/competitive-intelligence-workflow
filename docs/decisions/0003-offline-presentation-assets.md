...
- 状态：已实施并通过静态合同、Chromium/WebKit 真实运行和人工视觉复核
- 日期：2026-08-11
...
5. 用户已确认采用执行时最新版康哲规范；离线资产已写入项目，并由项目内设计合同约束视觉，不再同步读取通用规范。
...
- `assets/brand/cms-logo.svg` 与来源字节摘要一致，并有独立来源清单；
- `assets/third-party/echarts/` 固定 6.1.0 classic bundle、Apache-2.0 许可、npm 完整性和文件摘要；
- `assets/html-ppt/` 固定上游提交、MIT 许可、上游运行时摘要和两份衍生文件摘要；
- 静态合同验证 Logo、许可、来源、文件摘要、中文标签、离线边界及已删除的通用主题能力；
- Chromium 与 WebKit 均在 `file://` 下实际完成翻页、深链、页码、逐字稿、演讲者视图、计时、预览和视口缩放；
- ECharts 在两种浏览器中以本地 classic bundle 实际生成 SVG 图表，未发出 HTTP/HTTPS 请求；
- 人工打开观众页、演讲者视图和 WebKit 图表截图，未见内容叠压、不可读或非中文界面。

本项只接受离线运行基础设施，不等同于任何 A/B/C 报告、康哲报告页面或最终格式已完成。后续报告仍须使用项目康哲设计合同和本次运行来源包独立验收。
