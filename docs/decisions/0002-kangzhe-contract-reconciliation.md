...
- 状态：historical / superseded；当前项目自有设计合同决定见 ADR 0005
- 首次核对：2026-08-10
- 最新复核：2026-08-11
- 核对方式：差异分析前完整读取并记录元数据，差异分析后再次完整读取并复核同一组元数据

> 第 1–15 节保留历史候选、当时建议和 S1 失败审计，均不能再作为当前执行入口；用户后续决定见第 16 节，当前项目权威以 ADR 0005 为准。
...
真实视觉也不符合候选合同：第 11 页四张 KPI 卡的上下标签明显重叠；第 4、6、8、11 页存在 9–11.25 pt KPI 提示、流程标签和图例。`core.md` §0.7/I-03 的默认下限为 12 pt；只有明确标记 `data-density=ultra` 且属于表体或坐标轴时才可降至 10.5 pt，这些对象不满足例外。生成者的 `FRICTION.md` 明知全局最小为 9 pt，却引用历史 R9 母版将其自行解释为“辅助 hint”，并在无法查看 PNG 的情况下宣布通过；这不是合同允许的例外。
...
## 16. 用户后续决定：选取最新版并内化，不再同步通用版

2026-08-11 用户明确要求选取康哲最新版 `design.md`，内化为项目自己的 design spec，后续不再同步通用版。该后续指示解决了本文件第 15 节的权威阻断：本项目不再等待上游修订，也不修改共享模板；而是把稳定来源作为一次性输入，在新仓建立独立设计合同。

执行时确认四个最新版入口 stub 内容相同，均指向完整 `design_specs/`。对四个入口及 `design_specs/` 全部文件连续读取两次，path/hash/metadata 快照一致，稳定快照摘要为 `82a81132a0284f4bc984a582e0ad9a82de36e5b2ef065438a092b2cd9babc25b`。项目内运行入口为 `contracts/kangzhe/design.md`，完整权威为 `contracts/kangzhe/design_specs/`，来源与当前摘要见 `contracts/kangzhe/manifest.json`。

内化不是静默照搬缺陷：项目副本删除 `local_map` 的双全文同步要求，增加竞品调研 `project_profile.md` 和原生 PDF 轨，并明确门户使用 site+interactive、原生 PDF 使用 stream+pdf、HTML-PPT/PPTX 各自独立。暂停前 S1 的零产物、讲者运行时、跨页数字、来源外数字、过小字号、PPT Master 和自签完成问题，均转成项目合同和机械测试；完整决定见 [ADR 0005](0005-kangzhe-design-spec-repair-contract.md)。

从本节起，第 12、15 节“等待上游修复后再确认”的建议被取代；早期候选摘要和失败记录继续保留为 historical 证据。Task 0.3 的剩余工作是独立复核、离线资产封装和全量验证，而不是通用模板 S2 重跑。
