# 独立视觉验收审阅任务（报告 {REPORT} · abc-v2 · HTML 门户）

你是独立上下文视觉审阅者（身份 {VERIFIER}），与生产者（{PRODUCER}）相互独立。你的结论将以你的身份签署进正式视觉验收记录，必须基于你自己对渲染页面的真实检查。

## 工作目录
项目根（阵发性睡眠性血红蛋白尿症竞品调研，ABC 三报告 HTML 门户）。

## 材料（均相对项目根）
1. `packets/2026-09-18-omp-visual/visual-plan-{L}.json` — 视觉策划书（七验收域判据在 acceptance_matrix）
2. `packets/2026-09-18-omp-visual/render-evidence-{L}.json` — 真实呈现证据（30 个呈现目标：Chromium/WebKit × 5 视口 × 3 代表页，每目标含真实交互探测与截图路径）
3. `reports/{REPORT}/v1/html/` — 被验收站点（截图在 packets/2026-09-18-omp-visual/screenshots/）
...
## 产出（必须）
写文件 `packets/2026-09-18-omp-visual/visual-verdict-{L}.json`，UTF-8 JSON：
```json
{
  "schema_version": "1.0",
