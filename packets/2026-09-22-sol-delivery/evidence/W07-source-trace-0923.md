...
### 暂停前真实 PNH 安全批量核对与抗药抗体误判纠正

新增 `build_ctgov_a_safety_candidate_batch`：复用单条核证身份门，按来源一次重提取并区分 `no_exact_match`、`ambiguous`、`source_atom_reused`，仅输出未独立医学复核的开发候选。首次以两页 SHA-256 与文件名一致的原始 PNH CAS 和当前别名表运行通用构建，得到 3895 疗效/517 安全；其中 246 条来自 `outcomeMeasures`，241 条唯一绑定、5 条无匹配，另 271 条 `eventGroups` 不属于此绑定分支。细查 5 条均为 TEAE/SAE 等联合标题在 class 内报告的原始复合项，不能把它们分裂成独立 SAE/TEAE；先前根据行内空 n/N 猜测其中 3 条“来源没有分母”是不成立的，最终精确重提取显示这些来源有独立分母原子，只是复合概念行不得借分母计算风险率。修正后五条均可保留原复合身份并唯一绑定。另加合成缺分母用例，直接人数仍可引用原文，但风险率留未知。

邻接 AD 科学审计暴露安全域政策 7.5 将 `treatment-emergent ADA` 误判为 adverse event，额外制造 12 条错误的安全缺口；政策 7.6 改为需要明确 AE/TEAE/SAE 等术语。最终两页 PNH 同字节重建变成 3898 疗效/514 安全（3 条 ADA 回到非 AE 域），载荷 SHA-256 `93a629cfbcc84569b22af635df95e8a44f616e94dd633c649c43f752387c32c9`。在正式模型校验后的 243 条直接安全行上，**243 条唯一候选、459 条原子事实、243 条直接证据声明、0 条绑定缺口**；这些行所涉研究来源仍返回 **50 条 `parse_failure`**，不能把该计数外推到全部 50 NCT 或当作已经解释。余下 271 条 AE 事件组人数（包括死亡）尚未经过这条来源绑定，正式站点逐事实闭包仍 **0/4412**。新候选只在临时目录重放；没有正式 SQLite/快照、报告发布或浏览器/医学独立验收。

同轮相关整族初次为 **32 passed、1 failed**：失败是旧审计断言只预期 8 条真实缺口，政策误判多出 12 条；追查真实终点后修政策而非放宽断言，旧 AD 的 7 条 0/0 与 1 条 `numAffected` 缺失仍被科学门拒绝。最终测试与静态门结果待下方暂停记录固化。

最终源码相邻批 **34 passed in 94.75s**；Ruff 受影响 `src/tools/tests`、strict-mypy 全部 243 个 source files、diff 检查通过。更改后的三条 PNH ADA 分别来自 NCT03946748、NCT05886244、NCT04162470，终点原文明确为抗药抗体；它们在新候选疗效域而非 AE 安全域。旧 v106 与 W05A v6 字节及其 4412 分母未更改，不能把新候选数字覆盖旧门户正式来源门。

暂停前广门 `bash tools/gate.sh` 的准确口径为 `GATE_FAIL steps=6 failures=1`：活跃 unit/contract `1062 passed, 2 failed, 20 deselected`；保留轨兼容 smoke `20 passed`、分层审计 `7 passed`、Ruff、全源码 strict-mypy、旧路径检查通过。失败为早已存在的 `user_cleared` 枚举/旧测试不一致，以及 W03 冻结 manifest 中 6 个 `current_sources` SHA 与后续改动源码不一致；用 `git show 887667b` 确认失败条件在本轮改动前已存在。不更新冻结旧回执、不回退用户清除功能，也不把这两个 FAIL 归入 W07 相关 34 项通过。

### 直接报告安全测量的独立精确绑定

旧 AE 绑定只接受 `eventGroups` 的受影响人数/风险人数派生百分比，不能绑定 `outcomeMeasures` 中直接报告的安全人数、事件次数或百分比。现增加独立绑定及研究包提交前重提取：安全行保存原始 class/category 标题并逐项核对完整终点、组号和测量级组名、观察时间、来源原始单位、统计对象、原值及已存在的来源字段。测试中同终点同组同值的 `Any` 与 `Other` 两类目可区分，错类目、错访视、错单位、错数值、错分母和冲突路径均拒绝。人数 `8/35` 的两条原子事实分别持有原文字段路径，声明为直接报告而非算出风险率；事件 `41` 次与直接报告的 `20%` 各为单一来源值。合成小批经既有摄取器保存来源版本和精确事实片段，**不是**正式 PNH 数据库，也不是 517 条真实安全候选已绑定。

边界：通用解析器对人数项仍要求可核查同组分母，遇来源 0/0 不生成比率原子；没有分母时直接报告的人数虽在构建器候选中保留，但当前原子绑定仍留缺口。`numAffected` 缺失继续按用户裁决标记未知待核，绝不补零。真实 517 安全行尚未跑批量唯一身份统计、正式摄取或浏览器接受；W07 当前门户 **0/4412** 完整逐事实来源保持未闭合。

最终源码定向 **17 passed in 40.63s**，受改文件 Ruff、strict-mypy 3 个源文件和 `git diff --check` 通过；较早同轮 CT.gov 覆盖批 **30 passed in 95.65s**，不能跨版本合并为一个全门回执。
