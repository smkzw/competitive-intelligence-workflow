...
B 疗效/安全的新增来源视图须显式声明 `coverage_mode=complete|partial`：完整研究包由科学内容逐行证明 `complete`；逐事实补源的开发候选用 `partial`，视图行按相同 row ID 或领域行的显式 `source_view_row_id` 覆盖，未覆盖领域行继续可达。同一指针的产品、试验、单位、来源版本、数值和披露状态冲突则失败，不能按同值、位置或标题自动合并。缺声明的旧载荷仅沿用旧视图优先读取以保持历史兼容，不构成新版 B 全研究验收；发布验收必须核对来源视图/领域行的完整集合与显式关联。
