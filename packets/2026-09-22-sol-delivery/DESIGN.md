...
刷新候选的 A 结果消费行不得依赖分页、研究顺序或全局计数器。通用构建器以 NCT、精确原文路径和测量语义形成稳定行 ID，原值变更不改逻辑行身份；同一路径被不同结局/组别复用时语义变化产生新身份，碰撞失败关闭。内容寻址仓库同时含原始分页与逐研究派生时，必须用完整获取回执逐页钉住原始分页及研究集合，不能靠文件名排序读所有 blob。旧 ordinal 行 ID 的快照只读保留；旧→新映射需按精确研究/原文定位及上下文核验，不能因同标题/同数值自动移植用户修订或消费者。分页原始 hash 的变化不自动等于医学结果变化，动态状态与产品关系仍须独立比较。本条的当前 PNH 开发回执见 STATUS R24-42，尚非发布刷新验收。

CT.gov 的 `leadSponsor` 是**试验登记申办关系**，不是产品研发企业、许可方或权益地域。通用 A 载荷须按明确试验—实验臂—产品关系收集所有不同申办方，稳定排序，并以独立 `sponsor` 字段供企业页、产品页和关系图消费；不得因新增试验/分页顺序变化从多家首选一家并填入许可方。未有独立企业来源前，`developer_basis=registration_sponsor` 只支持“登记申办方（不等于研发归属）”措辞，真正研发归属保持待核。桌面 A 的未知靶点项目按阶段列出并可达，已核靶点计数不包含“未公开披露”等占位；首页先给竞争格局，再进入详细疗效，不把开发候选项目集合称为已闭包宇宙。R24-44 的单源开发页是这一规则的实现试片，非独立来源核验。

0924V1 R24-58 补充：只读刷新桥必须在比较前核对旧/新侧车、项目数据库、比较器源码与旧源纪元回执的冻结摘要；旧源回执同时证明旧侧车和快照摘要，当前消费者绑定的 `evidence_snapshot_id` 必须与项目内该快照逐字节一致。A 来源域 `additional_observations` 与消费者集合 `observations` 仅在显式映射下匹配，未知集合失败关闭。桥接结果只报候选/重建/三方冲突，绝不据候选直接迁移事实、注册消费者或切 current；历史回执不覆写。

R24-59 补充：任何新来源原子字段未被当前比较合同认识时，不得默默判为“科学原子未变”；旧绑定如经非用户编辑 supersedes 到活动事实，诊断桥先失败并要求显式三方谱系处理，不把它算作消失。不可变 SQLite 读取结束再查 WAL；只读候选门仍不等同正式刷新事务。
...
报告值已披露和逐事实来源已定位是两个不同状态。B 未覆盖的相关观察保留可检索性；没有来源版本、实际原文与精确 locator 三件套时，证据视图标为 `unverified`，不得用固定来源名、终点定义或泛登记页制造引文，也不能自动称科学结论已验。三件套齐全时标 `located` 并保留**该来源自己的** URL、原文和源内精确字段/页表段定位；NCT 标识不能把论文或监管 URL 改写成登记链接。`located` 仍只表示追溯材料齐备，不代替重提取、医学裁决或独立终审。
