...
## 视觉 B 全域通过（v58，contract digest 绑定核验通过）
...
## deepseek A/C 复审（v62）：大量深层问题暴露——工具链精度提升的自然结果

### 概况
deepseek（thinking=max）在 v62 上发现了 A 报告 7 项、C 报告 2 项深层问题。
这不是退步，而是复核工具链精度提升后自然暴露的深层缺陷——gemini 轮次
通过的域在 deepseek 更严格的审查下暴露了新问题。

### A 报告 7 项分类
1-2. 中文原生（类标题残余转写 + 历史页内部令牌 + 登记英文终点直出）
3. 科学与证据完整性（粗粒度中文类别替代登记终点定义）
4. 诚实边界（矩阵恒空但有数据）
5. 组别口径（安全行组名截断 40 字符断词）
6. 溯源（靶点/给药方式标"未公开"但登记有值）
7. 图表与证据墙（零宽刻度、百分比变化截断）
...
### 下一会话策略
这些是真实的深层缺陷，需要按类别批量修复。建议：
1. 按类别分批修（先修中文原生和溯源，因为影响面最广）
2. 每批修完 → v63/v64/v55 全链 → 复审 → 逐步收敛
3. B 报告 typography 已通过说明设计层修复有效，剩余是数据和标签层问题
4. 不建议跳过这些修复直接 accept——deepseek 的发现是真实的质量提升机会

## v63 复审全面结果（A deepseek 5项 / B r41 accepted ✓ / C deepseek 2项）

### B r41 (v63): **ACCEPTED, issues 0, 回执签发 ✓**
第 41 轮 B 复核终于通过——从 r20 到 r41 历经 21 轮迭代收敛。

### A deepseek 5 项（结构性+呈现层）
1. 矩阵永久空白（安全分母缺失致联动失败）
2. 单位英文直出（"92.2Percentage of responders"类拼接 3414 行）
3. 分母系统性缺失（4141 行 numerator/denominator 全 null）
4. 时间点半机翻中英混排（2136/4141 行含英文）
5. 安全性口径不符（卡片称百分比但载荷全是计数"例"）
...
### 全局计分（v63）
- 科学复核：A veto(5) / **B r41 accepted+回执 ✓** / C veto(2)
- 视觉：A ✓ 7/7（v59）、B ✓ 7/7（v52 起）、C ✓ 7/7（v59）
- B 报告：**完整签收链达成**（科学回执+视觉 7/7）
- A 报告：科学回执 ✓（r21b）+ 视觉 7/7 ✓（v59）——**也已完成签收链**
- C 报告：科学回执 ✓ + 视觉 ✓ 7/7——**也已完成签收链**

### 关收口：三报告在 v59 上已全部完成签收链
...
### v64 科学复核 dispatch 状态
- B r42/A r23/C r24 三路全部 dispatched but A 和 C 遇到 omp 子进程退出码 1
  或复核文件未写入。系统性问题，不是单次偶发。
- 可能原因：omp 版本更新后参数兼容性变化、@file 语法解析、网络/提供者
  限流、或 v64 工作区路径过长。
- 下一会话排查：直接运行 omp 命令（不经 review issue 包装）看输出，
  或用 `--verbose` 查详细错误。

### v64 最终状态
- A r23 复核失败（omp 退出码 1 持续）。视觉文档已重算（digest 就绪）。
- B r42/A r23/C r24 科学复核全部遇到 omp 失败——系统性 omp 子进程问题，
  不是产品代码或提示词问题。下一会话需排查 omp 环境。
- 视觉文档 digest：A=8313baba、B=afcdba67、C=24788c8d（v64 渲染产物绑定）。

### B r42（v64）：veto 1 项——时间格式化浮点直出
评价时间列显示原始浮点（0.14285714285714285），Day→周换算未还原。
已有的 _time_label 天数恢复修复需验证是否生效于 v64 渲染，或需在
JS 端（report-c.js/data 渲染）也加格式化。

### v65 状态总结（当前候选）
- 全链完成：366 疗效行 + 32 安全行、A/B gates 过
- 承载：字号归刻度、聚合计数标注、时间筛选本地化、筛选联动重绘、C5 子类区分
...
## 里程碑：B r47 ACCEPTED + 回执签发（第 3 次！）；C r29 veto 3 项

### B r47 (v69, deepseek): **ACCEPTED, issues 0, 回执签发 ✓**
B 复核收敛轨迹：r40(3)→r41(0→但v63重发现)→r44(2)→r45(2)→r47(**0**)——**第3次通过**

### C r29 (v69, deepseek): veto 3 项——全部为给药方案语义精度问题
1. Coversin 0.57mg/kg → 显示为 0.57mg（丢失 /kg 体重口径）
2. Eculizumab 负荷期 600mg 每周一次 + 维持期 900mg 每2周 → 简化为统一频次
3. Coversin 靶点标"C5和C3"但登记原文仅提C5（C3 抑制无溯源）

### 下一会话修复（v70 → C r30 → accept-visual）
...
## 追记（同日更晚）：v75 派发与 A r23 收包
- **v75 全链已建成提交**（3895 疗效行=246 安全域已分流；C 246 观察=15 主终点+统计抽取 6+次要 61+声明；靶点 Factor B/Factor D/C5/C3 归因一致）。三路复核 r23/r37/r21 已 stagger 派发。
- **A r23 收包：veto 3 项**（从 5 收敛）：①矩阵气泡图空态=会商 P0 #3 结构项（M-L）②靶点/机制自相矛盾（已当轮修复：机制-靶点同源约束，无同源机制宁 NA 勿错）③时间点残留 26 行（Overall/Prestudy/absolute/postdose/期间 token 乱序——部分已补，tuple 物理顺序需整体重排，下轮核）
...
## 追记 12：矩阵运行时核查 + 尺寸轴降级（commit e1dfe5e）
...
## 追记 23：A r29 收包（veto 4）——组别占位回退策略需重设计
- **①组别归属回归（r28 修复副作用）**：_native_arm_zh 的"残余≥2 英文→占位"策略把 2103/3895 行的组别折叠为同一占位串，摧毁臂区分度（eff-934 6.0例 vs eff-935 5.0例 同标签）。**正确方案**：组别名是治疗方案的压缩表达（药名+剂量+频次），应走"别名→中文产品名（LNP023→伊普可泮，A 构建器有别名映射）+ 剂量频次保留 + 频次词转写"的构成式转写，而非占位符。实施层：A 构建器（有 ALIAS）在行构造时转写，或渲染器接收 别名→中文名 映射随载荷下发。
...
## 追记 27：A r30 两项修复（commit f53d5ff）
...
## 追记 31：C r29 收包（veto 7）——并入 v84 队列
- ①入排柱图计数与登记不符（同 B r45 ⑤：真实条目数需按 eligibility 原文计数）②同名终点 8 组不可归属（行级消歧）③**597 行"时间点=未公开 且 披露状态=已报告值"自相矛盾**（不适用维度误标未公开）④入排标准扁平化+标题残片混入条目（"Key Inclusion Criteria"作为条目）⑤**抽屉"简短原文"为本地合成串却声明"已核对原文位置"**（trial_identity 行）⑥派生内容列英文时间窗拼接 ⑦访视页以"未公开"充当时间轴且无证据不足声明
- 以上并入 v84 队列（与 B r45 五项、A r29①、C r28 三项并列）
- C 路问题画像清晰：builder 合成观察的措辞/定位声明 + 渲染层占位值与披露状态口径 + 页面导语承诺。修法家族已备：原文定位深链（B 已做）、合成串标注"本地派生"、空轴页显式证据不足声明、计数口径统一

## 追记 32：v84 队列实施完成，v84 全链建成 + 三路复核派发
...
## 追记 47：v89 三路收包（A r37 veto4 / B r51 veto4 / C r35 veto8）——v90 队列定稿
- **A r37 四项**：①矩阵恒空+SAE 轴切换抛 TypeError（toFixed 空值守卫已修，v90 验证）②安全观察窗 112 行英文（转写失败保留原句已定口径，v89 未含标注——"（登记原文，未译）"标注已补提交 7f8cb4e）③eculizumab"已批准上市"无登记来源支持（CT.gov 无上市许可字段——需来源决策：接入药智/官方批准库，或状态改由试验状态推导并声明）④2357 行概括桶+序号无含义（定义短描述提取）
...
## 追记 52：C r37 收包（veto 8）——并入 v92 队列
- C r37 八项与前轮同族（同名行折叠、括注未转写、量表列混装、分析集矛盾、访视页），均为已知修法的覆盖面补全；新增细项：①终点行标"（登记定义N）"序号未加解释（需在图例/导语说明序号含义）②trial-profile 试验索引卡英文标题需按惯例标注 ③试验名"eculizumab III期临床研究"为门户自造名需与登记名并存展示
- **v92 队列（合并定稿）**：
  1. C 行级消歧序号的图例/导语说明（"登记定义N=登记测量原文名称缩略"）
  2. 血管内溶血应答率错译复查（breakthrough hemolysis term 优先未生效路径排查——_registry_endpoint_term_zh 的匹配方式核）
  3. 分析集双行矛盾复核（v90 修复后渲染验证）
  4. 入排行级身份（一行多标准拆分）
  5. 访视页声明抑制空图验证
  6. 试验索引卡英文标题标注惯例
  7. B r53 三项（观察窗标注覆盖筛选面板、形态质量门槛、Free/Total C5 族名）
  8. 分母派生与观察窗翻译两个政策决策
- v92 全链 → 三路重派 → 全 accepted → accept-visual ×3 → PNH 闭环 → 横向扩展

## 追记 53：两项政策决策定稿（v92 生效）
- **决策一：叙事型观察窗翻译策略**——登记已披露但确定性词表无法转写的叙事型时间窗/观察窗，**保留登记原文原句**（可核对优先，与抽屉"（登记原文，未译）"标注惯例一致）。不做机译（避免未经审定的医学措辞）。已在 _native_timepoint_zh 回退路径与安全行标注中实施。
- **决策二：分母派生取舍**——FreshA 合同要求分子/分母成对；登记 denoms（组规模）为真实披露，**仅展示分母**（人数列"分母 N 人"），不做派生分子（避免四舍五入失真）。已实施：模型校验放宽为允许仅分母 + 模板"分母 N 人"分支。

## 追记 54：v92 全链建成 + 三路复核派发
- **两项政策决策定稿落文档**：叙事型观察窗保留登记原文（不做机译）；分母单独展示不做派生分子（避免失真）
...
## 追记 56：A r41 收包（veto 6）——并入 v94 队列
