...
- Do not write the runner-managed report path `runs/conference/ci-phase9-task93-optional-monitoring-review/general_single_object.md`; return the complete report for the runner.

Initial read set:
- `context/ci-phase9-task93-optional-monitoring-review_conference_context.md`
- `plans/codex_main_venue_ci-phase9-task93-optional-monitoring-review.md`

Objective:
独立审查 Task 9.3 当前实现：候选是否可能越权成为事实或触发发布，去重与 model_copy 防漂移是否可靠，技术失败/未发现/未公开是否真实区分，用户处置和刷新交接是否只读且可恢复，监测是否可卸载，中文提示是否面向医学经理。只给出可复现问题与通过条件，不修改代码。
...
Output schema:
1. `# Conference Participant Output: ci-phase9-task93-optional-monitoring-review - general_single_object`
2. `## Boundary Check`
