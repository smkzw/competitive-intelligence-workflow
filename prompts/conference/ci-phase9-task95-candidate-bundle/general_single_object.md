...
- Runner-managed report path: `runs/conference/ci-phase9-task95-candidate-bundle/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/ci-phase9-task95-candidate-bundle_conference_context.md`
- `plans/codex_main_venue_ci-phase9-task95-candidate-bundle.md`
...
Objective:
对已完成修复的 Task 9.5 候选 Skill 包做治理关联复审：核实 bundle 内容闭包、规范 fresh-install、Codex/Hermes/OMP 三真实宿主完整无草稿阻断到补件恢复和站点式 HTML、中文安装说明及当前摘要归档；沿用既有独立审阅会话，不重做实现。
...
Output schema:
1. `# Conference Participant Output: ci-phase9-task95-candidate-bundle - general_single_object`
2. `## Boundary Check`
