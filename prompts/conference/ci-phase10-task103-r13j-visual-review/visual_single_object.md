...
- Runner-managed report path: `runs/conference/ci-phase10-task103-r13j-visual-review/visual_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/ci-phase10-task103-r13j-visual-review_conference_context.md`
- `plans/codex_main_venue_ci-phase10-task103-r13j-visual-review.md`
...
Objective:
从资深中文临床试验医学经理视角，使用 ego(lite) 真实浏览 Task 10.3 R13j A/B/C 站点，独立审评交互、横向比较、设计细节、中文表达、默认视野可读性与康哲设计一致性；只验收 HTML，不涉及 PDF/PPT。
...
- `pi` / `kimi-code` / `kimi-for-coding` / effort high
- `pi` / `openai-codex` / `gpt-5.6-terra` / effort medium

Output schema:
1. `# Conference Participant Output: ci-phase10-task103-r13j-visual-review - visual_single_object`
2. `## Boundary Check`
