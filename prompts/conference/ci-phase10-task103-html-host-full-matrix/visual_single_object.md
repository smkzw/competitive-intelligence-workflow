...
- Runner-managed report path: `runs/conference/ci-phase10-task103-html-host-full-matrix/visual_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/ci-phase10-task103-html-host-full-matrix_conference_context.md`
- `plans/codex_main_venue_ci-phase10-task103-html-host-full-matrix.md`
...
Objective:
对 Task 10.3 R13k 站点式 HTML 全矩阵候选做独立视觉终审，重点确认 B 基线分页与处置标题修复、A/C 无旁路回归、ego(lite) 证据边界；只读，不修改候选，不宣称正式发布。
...
Output schema:
1. `# Conference Participant Output: ci-phase10-task103-html-host-full-matrix - visual_single_object`
2. `## Boundary Check`
