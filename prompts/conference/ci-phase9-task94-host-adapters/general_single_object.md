...
- Do not write the runner-managed report path `runs/conference/ci-phase9-task94-host-adapters/general_single_object.md`; return the complete report for the runner.

Initial read set:
- `context/ci-phase9-task94-host-adapters_conference_context.md`
- `plans/codex_main_venue_ci-phase9-task94-host-adapters.md`

Objective:
独立审阅 Task 9.4 三宿主薄适配器、统一回执和真实源码级宿主冒烟：重点挑战宿主越权、三宿主语义漂移、回执可伪造性、关键证据不足时零草稿、恢复与故障分类、中文用户状态，以及源码级验证与 Task 9.5 最终安装验收的边界；仅依据文件和实际检查给出缺陷与是否可关闭结论。
...
Output schema:
1. `# Conference Participant Output: ci-phase9-task94-host-adapters - general_single_object`
2. `## Boundary Check`
