...
- Runner-managed report path: `runs/conference/ci-phase9-task95-candidate-bundle-review/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/ci-phase9-task95-candidate-bundle-review_conference_context.md`
- `plans/codex_main_venue_ci-phase9-task95-candidate-bundle-review.md`
...
Objective:
独立审阅 Task 9.5 候选 Skill 包：挑战 bundle 内容闭包、隔离安装、Codex/Hermes/OMP 真实宿主进程回执、no-draft 语义、首版仅站点式 HTML 边界及中文安装说明；不得把版本探针或直接 ci-workflow 子进程冒充真实宿主通过。
...
Output schema:
1. `# Conference Participant Output: ci-phase9-task95-candidate-bundle-review - general_single_object`
2. `## Boundary Check`
