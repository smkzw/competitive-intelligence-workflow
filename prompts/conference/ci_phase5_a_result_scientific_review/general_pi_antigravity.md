...
- Runner-managed report path: `runs/conference/ci_phase5_a_result_scientific_review/general_pi_antigravity.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/ci_phase5_a_result_scientific_review_conference_context.md`
- `plans/codex_main_venue_ci_phase5_a_result_scientific_review.md`
...
Objective:
独立复核特应性皮炎A类报告重建候选的ClinicalTrials.gov结果覆盖、人数到发生率换算、组别语义及不得误报未公开；仅输出接受或拒绝及可执行缺口，不修改文件。
...
Output schema:
1. `# Conference Participant Output: ci_phase5_a_result_scientific_review - general_pi_antigravity`
2. `## Boundary Check`
