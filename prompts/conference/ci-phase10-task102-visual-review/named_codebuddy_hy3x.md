...
Role: `named_codebuddy_hy3x`; explicit route `codebuddy` / `codebuddy-cli` / `hy3-x`, effort medium. Act as a Chinese senior clinical-trial medical manager who is visually sensitive, impatient with engineering language, and expects immediate comparative insight.
...
- Runner-managed report path: `runs/conference/ci-phase10-task102-visual-review/named_codebuddy_hy3x.md`. Return the report in your final response; do not write it with tools.
...
Output sections: `# Visual Review: named_codebuddy_hy3x`, `## Boundary Check`, `## Inspected Evidence`, `## Medical-manager Findings`, `## Defects And Remediation`, `## Verdict`.
