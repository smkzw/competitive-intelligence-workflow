...
- Task id: `ci_phase5_a_data_matrix_recheck`
- Role id: `worker_01`
...
- Runner-managed report path: `runs/execution/ci_phase5_a_data_matrix_recheck/worker_01.md`. Never invoke write/edit tools
...
Objective:
复查并修复A类特应性皮炎报告数值覆盖不足与首页/安全性页热图默认视口横向溢出，形成可追溯新产物

Task:
Execute only this assigned work item: 只读审计当前38个产品/43项试验从来源到默认图表的数值覆盖，逐产品定位真实未公开、来源未检索、获取失败、解析遗漏、投影遗漏和默认筛选遗漏，并写审计报告，不修改代码或数据。
...
Output schema:
1. `# Execution Output: ci_phase5_a_data_matrix_recheck - worker_01`
