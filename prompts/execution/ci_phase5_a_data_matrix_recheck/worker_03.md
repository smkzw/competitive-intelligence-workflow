...
- Runner-managed report path: `runs/execution/ci_phase5_a_data_matrix_recheck/worker_03.md`. Never invoke write/edit tools
...
Task:
Execute only this assigned work item: 负责数值完整性数据链路：基于现有锁定来源和允许的多来源策略，修复产品/试验覆盖审计、错误结果状态或投影/默认选择逻辑，增加失败测试；不得修改门户布局CSS。
...
Output schema:
1. `# Execution Output: ci_phase5_a_data_matrix_recheck - worker_03`
