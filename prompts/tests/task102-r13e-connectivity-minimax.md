...
- Runner-managed output path: `runs/tests/r13e-connectivity/minimax.md`。不得用工具写入该路径，由 runner 持久化最终回答。

Read these files only:
- `context/ci-phase10-task102-r13e-visual-review_conference_context.md`

Create/write only this output file:
- `runs/tests/r13e-connectivity/minimax.md`（runner 管理；只返回正文，不用工具写入）

预期身份：`pi/cms-router/minimax-m3:high`。

任务：
1. 明确报告实际代理、provider、model 与 effort；若无法确认，写“无法确认”。
2. 使用 `ego-browser nodejs`，创建或复用名为 `ci-r13e-connectivity-minimax` 的任务空间，打开 `http://127.0.0.1:8770/b-real/reports/B/v1/html/overview.html`，读取页面标题和 URL。
