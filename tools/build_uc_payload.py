...
args = [
    ...
    "--cas-dir", str(ROOT / ".artifacts/source-cas/ctgov-uc-20260920"),
    "--alias-map", str(ROOT / "packets/2026-09-20-test-round-2/uc-alias-map.json"),
    "--indication", "溃疡性结肠炎",
    "--indication-id", "uc",
    "--output", str(ROOT / "packets/2026-09-20-test-round-2/uc-a-payload.json"),
    "--cutoff", "2026-09-20",
]
alias_path = ROOT / "packets/2026-09-20-test-round-2/uc-alias-map.json"
if not alias_path.exists():
    data = json.load(open(ROOT / "packets/2026-09-20-test-round-2/uc-page-1.json"))
    drugs = set()
        ...
        for iv in (s.get("protocolSection", {}).get("armsInterventionsModule", {}).get("interventions") or []):
            if (iv.get("type") or "").upper() in ("DRUG", "BIOLOGICAL"):
                ...
                if name and name.lower() not in ("placebo", "matching placebo"):
                    drugs.add(name)
    alias = {
        "schema_version": "1.0", "map_id": "uc-alias-auto",
