...
args = [
    ...
    "--cas-dir", str(ROOT / ".artifacts/source-cas/ctgov-igan-20260920"),
    "--alias-map", str(ROOT / "packets/2026-09-20-test-round-2/igan-alias-map.json"),
    "--indication", "IgA肾病",
    "--indication-id", "igan",
    "--output", str(ROOT / "packets/2026-09-20-test-round-2/igan-a-payload.json"),
    "--cutoff", "2026-09-20",
]
# 创建最小别名表（如果不存在）
...
if not alias_path.exists():
    # 从 CT.gov 数据自动提取药物名
    data = json.load(open(ROOT / "packets/2026-09-20-test-round-2/igan-page-1.json"))
    drugs = set()
    for s in data.get("studies", []):
        for iv in (s.get("protocolSection", {}).get("armsInterventionsModule", {}).get("interventions") or []):
            if (iv.get("type") or "").upper() in ("DRUG", "BIOLOGICAL"):
                ...
                if name and name.lower() not in ("placebo", "matching placebo"):
                    drugs.add(name)
    alias = {
        "schema_version": "1.0", "map_id": "igan-alias-auto",
