...
def test_free_hemoglobin_and_transfusion_burden_are_distinct_families() -> None:
    ...
    result = classify_registry_endpoint(
        "Number of RBC Transfusion Instances During 12 Weeks"
    )
...
def test_percent_change_free_hgb_and_burden_change_are_distinct() -> None:
    ...
    assert classify_registry_endpoint(
        "Percent Change In Free Hemoglobin Levels From Baseline"
    ) == "endpoint-pnh-free-hemoglobin-pct-v1"
    assert classify_registry_endpoint(
        "Change in Number of Transfusion Instances From 24 Weeks Prior"
