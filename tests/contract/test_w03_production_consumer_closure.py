...
from ci_workflow.application.fresh_c_research_package import derive_c_research_facts
...
from ci_workflow.application.source_research_service import ResearchClaim, ResearchFact, SourceCapture
...
def test_pnh_b_builder_emits_closed_typed_nonpercent_matrix_projection() -> None:
    ...
    common = {
        "product_id": "drug-x", "trial_id": "nct-w03", "endpoint_family_id": "hb",
        "actual_timepoint": 24.0, "analysis_population": "ITT", "unit": "g/L",
        "analysis_form": "change_from_baseline", "direction": "higher_is_better",
    }
    matrix = builder.build_typed_matrix_view(
        ...
        [{
            "trial_id": "nct-w03", "arm_role": "treatment", "count_basis": "participants",
            "value": 12, "numerator": 12, "denominator": 40, "unit": "例",
...
def test_b_safety_term_key_survives_builder_payload_filter_and_table() -> None:
    builder = _load_builder("build_pnh_b_audit.py")
    typed_row = {
        "row_id": "typed-absence-sae", "product_id": "iptacopan",
        "trial_id": "nct04558918", "family": "sae", "source_term": "No SAEs",
        "term_key": "absence_sae", "polarity": "negative_presence", "grade_set": [],
        "seriousness": "serious", "teae": False, "relatedness": "unspecified",
        "parent": None, "children": [], "count_basis": "participants",
        "at_risk_stat": "serious", "arm_label": "Iptacopan", "value": 0,
        "numerator": 0, "denominator": 62, "unit": "%",
        "time_window_zh": "24周治疗期",
    }
    ...
    table_rows = _page_records(
        data, page_id="safety", names=names, trial_names=trial_names,
        efficacy=(), safety=records,
    )
...
def test_production_b_c_builder_rows_roundtrip_w01_and_manifest_only_restore(tmp_path: Path) -> None:
    ...
    payload = {
        "studies": [{
            "protocolSection": {
                ...
                "outcomesModule": {"primaryOutcomes": [{"measure": "Response", "timeFrame": "Week 12"}]},
            },
    ...
    b_row = next(b_builder.b_facts({"efficacy": [{
        "row_id": "eff-1", "product_id": "drug-x", "trial_id": "nct-w03",
        "endpoint": "Response", "timepoint": "Week 12", "unit": "人",
        "arm": "Drug X", "value": 12, "population": "ITT",
        "source_field_path": "$.studies[0].resultsSection.outcomeMeasuresModule.outcomeMeasures[0].classes[0].categories[0].measurements[0].value",
        "source_text": "12",
    }]}))
    c_row = c_builder._row(
        "nct-w03", "drug-x", "NCT-W03", 1, "endpoint", "primary_endpoint_definition",
        "Response", seq="pri0", field_path="$.studies[0].protocolSection.outcomesModule.primaryOutcomes[0].measure",
        source_value="Response", endpoint_key="primary", outcome_id="pri0", period="overall",
        timepoint="Week 12",
    )
    acquired = datetime(2026, 9, 22, tzinfo=UTC)
    source = SourceCapture(
        source_id="prod-source", route_id="production-pnh-builders", source_type="clinical_trial_registry",
        title="Production PNH builder replay", url="https://clinicaltrials.gov/", query_or_identifier="NCT-W03",
        language="en", access_method="persisted_fixture", content_text=content_text,
        acquired_at=acquired, published_at=None, effective_at=None, first_disclosed_at=acquired,
        locator=EvidenceLocator(document_role="registry-search-page", field_path="$.studies"),
    )
    facts = (
        ResearchFact(
            fact_id="prod-b", row_ref=b_row["row_id"], entity_id="nct-w03", entity_type="trial",
            canonical_name="Response", field_id="result.efficacy", raw_value="12", normalized_value="12",
            disclosure_state="reported_value", source_id="prod-source",
            locator=EvidenceLocator(document_role="registry-search-page", field_path=b_row["source_field_path"]),
            original_text=b_row["source_text"],
        ),
        ResearchFact(
            fact_id="prod-c", row_ref=c_row["row_id"], entity_id="nct-w03", entity_type="trial",
            canonical_name="Primary endpoint", field_id="design.endpoint", raw_value="Response",
            normalized_value="Response", disclosure_state="reported_value", source_id="prod-source",
            locator=EvidenceLocator.model_validate(c_row["source_locator"]), original_text=c_row["source_text"],
        ),
    )
    project = tmp_path / "ingested"
    apply_migrations(project / "state/project.sqlite")
    lineage = ingest_research_evidence(
        project_root=project, project_id="w03-prod", contract_version=1, report_kind="B",
        data_cutoff=acquired, scientific_content_digest=sha256(content_text.encode()).hexdigest(),
        created_at=acquired, sources=(source,), route_attempts=(), facts=facts,
        claims=(ResearchClaim(claim_id="prod-chain", claim_text="B/C production rows replay",
                              claim_kind="direct_evidence", fact_ids=("prod-b", "prod-c")),),
    )
...
def test_fixed_cas_a_to_b_c_full_chain_roundtrips_every_fact_and_site_bytes(
    ...
    contract = create_project_contract(
        indication="阵发性睡眠性血红蛋白尿症", reports=["B", "C"],
        outputs=["html"], cutoff="2026-09-06",
    )
    ...
    assert all(row.get("source_field_path") and row.get("source_text") is not None
    ...
    for report, package in packages.items():
        ...
        lineage = ingest_research_evidence(
            project_root=ingest_root, project_id=contract.project_id,
            contract_version=contract.contract_version, report_kind=report,
            ...
            sources=package.sources, route_attempts=package.route_attempts,
            facts=facts, claims=package.claims,
        )
        exported = tmp_path / f"{report}-manifest.json"
        exported.write_bytes(
            (ingest_root / lineage.evidence_snapshot.relative_path).read_bytes()
        )
...
def test_w03_browser_freeze_rebinds_report_data_sources_and_screenshots() -> None:
    ...
    assert sha256(sitemap.read_bytes()).hexdigest() == (
        manifest["generated"]["retained_sitemap_sha256"]
    )
    report_source = report_js.read_text(encoding="utf-8")
    report_payload = json.loads(
        report_source.removeprefix("window.REPORT_C=").removesuffix(";\n")
    )
    ...
    assert page_bindings["endpoint-timepoint-matrix"]["route"] == (
        "/c/endpoint-timepoint-matrix"
    )
