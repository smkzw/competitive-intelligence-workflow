"""Task 9.4 HA03：Hermes 薄适配器真实行为建模。

最小输入、中断、恢复与产物四类映射全部经共享基类对公共设施的调用
发生；本测试独立驱动真实 API 并断言 Hermes 运行身份不进入语义面。
"""
...
def test_hermes_adapter_maps_minimal_input_interrupt_resume_and_artifacts(
    tmp_path: Path,
) -> None:
    adapter = HermesHostAdapter(probe=StaticCapabilityProbe())
    ...
    assert pending_item.inbox_directory == request["inbox_directory"]
    ...
    assert any(
        "补充可公开核对" in action
        for action in blocked.blocking_and_recovery.user_actions_zh
    )

    # ── 恢复映射：同一项目 + 环境修复只重排失败能力下游 ──────────────────
    broken = HermesHostAdapter(probe=StaticCapabilityProbe(blocked=("http_network",)))
    ...
    # ── Hermes 运行身份：候选定位与通道（不进入语义回执） ───────────────
    identity = adapter.run_identity()
    assert identity.host == "hermes"
    assert identity.host_label_zh == "Hermes"
    assert identity.session_channel == "hermes-cli-session"
    assert (Path.home() / ".hermes") in adapter.install_entry_candidates()
