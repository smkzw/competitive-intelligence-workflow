"""Task 9.4 HA04：OMP（Oh My Pi）薄适配器真实行为建模。

最小输入、中断、恢复与产物四类映射全部经共享基类对公共设施的调用
发生；本测试独立驱动真实 API 并断言 OMP 运行身份不进入语义面；宿主
私有会话丢失后仍可从项目文件重建同一语义。
"""
...
def test_omp_adapter_maps_minimal_input_interrupt_resume_and_artifacts(
    tmp_path: Path,
) -> None:
    adapter = OmpHostAdapter(probe=StaticCapabilityProbe())
    assert adapter.host == "omp"
    ...
    assert pending_item.reason_zh == request["reason_zh"]
    ...
    #    宿主私有会话丢失后从项目文件重建同一语义 ────────────────────────
    broken = OmpHostAdapter(probe=StaticCapabilityProbe(blocked=("http_network",)))
    previous = broken.capability_matrix(
        project_root=project, selection=selection(reports=("A", "B"))
    )
    assert previous.overall_state == "blocked"
    rebuilt = OmpHostAdapter(probe=StaticCapabilityProbe())
    recovered = rebuilt.build_semantic_receipt(
    ...
    assert recovered == adapter.build_semantic_receipt(
        project_root=project,
        selection=selection(reports=("A", "B")),
        previous_matrix=previous,
    )
    ...
    # ── OMP 运行身份：候选定位与通道（不进入语义回执） ──────────────────
    identity = adapter.run_identity()
    assert identity.host == "omp"
    assert identity.host_label_zh == "Oh My Pi"
    assert identity.session_channel == "omp-cli-session"
    assert (Path.home() / ".omp") in adapter.install_entry_candidates()
