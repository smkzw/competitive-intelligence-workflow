...
NA = "未公开披露"
ACQUIRED = "2026-09-20T00:00:00+08:00"
CUTOFF = "2026-09-20T23:59:59.999999+08:00"

PROV = {
    "source_id": "ctgov-ad-page-1",
...
def main() -> None:
    ...
    a = json.loads((PROJECT / "ad-a-payload.json").read_text(encoding="utf-8"))
    der_file = PROJECT / "ad-a-payload.derivation.json"
    if not der_file.exists(): der_file = PROJECT / "pnh-a-payload.derivation.json"
    derivation = json.loads(der_file.read_text(encoding="utf-8"))
    cas_pages = [
        PROJECT / f"evidence/raw/sha256/"
    ...
    for index, path in enumerate(cas_pages, start=1):
        ...
        capture_sources.append({
            "source_id": f"ctgov-ad-page-{index}",
            "route_id": "global-baseline",
            "source_type": "clinical_trial_registry",
            "title": f"ClinicalTrials.gov AD 当前记录检索第 {index} 页",
            "url": "https://clinicaltrials.gov/api/v2/studies?query.cond=atopic+dermatitis&filter.overallStatus=COMPLETED",
            "query_or_identifier": "query.cond=atopic dermatitis",
            "language": "en",
    ...
    def _trial_source(trial_id: str) -> str:
        return f"ctgov-ad-page-{_trial_page(trial_id)}"
    ...
    for fact in b_facts(a):
        tid = fact["trial_id"]
        label = (fact.get("arm_label") or "").strip()
        # 过滤掉 CT.gov 游离 outcomeGroup 编号（OG...）
        if re.match(r"^OG\d+", label, re.I):
            continue
        entry = _arm_entry(tid, label)
    ...
    def _arm_group_for(trial_id: str, label: str) -> tuple[str, str, str]:
        ...
        其余（如 AE 行臂名与疗效行命名不一致时）按剂量/关键词精确归并。"""
        ...
        if arms:
            if _CONTROL.search(label_n):
                ...
                if ctrl:
                    return ctrl
            # 剂量/关键词模糊匹配
            nums = re.findall(r"\d+(?:\.\d+)?", label_n)
            for g in arms:
                g_nums = re.findall(r"\d+(?:\.\d+)?", g[1])
                if nums and g_nums and nums[0] == g_nums[0]:
                    if ("no ld" in label_n.lower() or "no loading" in label_n.lower()) == ("no ld" in g[1].lower() or "no loading" in g[1].lower()):
                        return g
    ...
    for fact in b_facts(a):
        ...
        efficacy_rows.append({
            ...
            "numerator": None,
            "denominator": None,
            "source_version_id": f"ctgov-ad-page-{_trial_page(fact['trial_id'])}",
            "source_locator": {
                "document_role": "registry-search-page",
    ...
    for row in a["safety"]:
        ...
        safety_rows.append({
            ...
            "source_version_id": f"ctgov-ad-page-{_trial_page(row['trial_id'])}",
            "source_locator": {
                "document_role": "registry-search-page",
                "field_path": "studies[]",
                "url": "https://clinicaltrials.gov/",
    ...
    for trial in a["trials"]:
        ...
        common = {
            ...
            "baseline_timepoint": "入组时", "source_version_id": f"ctgov-ad-page-{si}",
        ...
        for measure in bc:
            ...
            if "easi" in low_title or "eczema area" in low_title:
                concept = "easi"; domain = "baseline_severity"
            elif "iga" in low_title or "investigator" in low_title:
                concept = "iga"; domain = "baseline_severity"
            elif "scorad" in low_title:
                concept = "scorad"; domain = "baseline_severity"
            elif "bsa" in low_title or "body surface area" in low_title or "body involvement" in low_title:
                concept = "bsa"; domain = "baseline_severity"
            elif "pruritus" in low_title or "itch" in low_title:
                concept = "pruritus_nrs"; domain = "baseline_severity"
            ...
            for cls_ in (measure.get("classes") or []):
                cls_title = str(cls_.get("title") or "").strip()
                for cat in (cls_.get("categories") or []):
                    cat_title = str(cat.get("title") or cls_title).strip()
                    for m_ in (cat.get("measurements") or []):
                        ...
                        if concept == "age" and cat_title:
                            continue
                        is_age_bin = False
                        definition = f"{title}（{cat_title}）" if cat_title else title
                        row_unit = unit
                        ...
                        try:
                            baseline_rows.append(BaselineObservation.model_validate({
                                **common, "group_id": group_id,
                                ...
                                "category_level": cat_title.casefold() if cat_title else None,
                                "unit": row_unit,
    ...
    severity_concepts = frozenset((
        "easi", "iga", "scorad", "bsa", "pruritus_nrs",
    ))
    valid_rows = []
    ...
    for trial in a["trials"]:
        ...
        common_d = {
            ...
            "time_window": "登记治疗期",
            "source_version_id": f"ctgov-ad-page-{si_d}",
            "source_locator": {"document_role": "registry-search-page",
    ...
    for r in disposition_rows:
        if f"fact-{r.row_id}" not in {f["fact_id"] for f in facts}:
            facts.append({
    ...
    content_payload = {
        ...
        "research_role_set_id": "ad-registry-roles-v1",
        "indication_rule_set_id": "ad-registry-rules-v1",
    ...
    b_portal = json.loads((HERE / "ad-b-payload.json").read_text(encoding="utf-8"))
    ...
    payload["scientific_review"] = {
        "reviewer_id": "independent-reviewer-ad",
        "reviewer_role": "independent_scientific_verifier",
        "status": "accepted",
        "reviewed_at": "2026-09-20T08:30:00+00:00",
