"""AD 首验：研究审计包 + A 报告包构建（LOOP 第十一轮）。

材料：ad-a-payload.json（真实派生 A 载荷）+ 两页 CT.gov CAS。
...
def main() -> None:
    ...
    payload = json.loads((PROJECT / "ad-a-payload.json").read_text(encoding="utf-8"))
    # 独立复核修复（第十二轮）：nct→页映射，事实来源页码按实际检索页绑定
    nct_page = {}
    _der_f = PROJECT / "ad-a-payload.derivation.json"
    if not _der_f.exists(): _der_f = PROJECT / "pnh-a-payload.derivation.json"
    _der = json.loads(_der_f.read_text(encoding="utf-8"))
    _cas_root = PROJECT / "evidence/raw/sha256"
    ...
    # 回执只到日精度（observed_date=2026-09-20）；按 date-identity 合同用日级保守表示，不虚构具体时刻。
    ...
    for index, (path, blob) in enumerate(page_blobs, start=1):
        ...
        audit_sources.append({
            ...
            "url": (
                "https://clinicaltrials.gov/api/v2/studies?query.cond=atopic+dermatitis&filter.overallStatus=COMPLETED"
            ),
            "locator": "studies[]",
            "locator_detail": {
    ...
    entities = [
        {
            ...
            "identity_evidence_ids": ["ctgov-ad-page-1"],
        }
        for pid in product_ids
    ]
    def _receipt(dim: str, discovered: list[str]) -> dict:
        return {
            "receipt_id": f"ad-{dim}-1",
            "dimension": dim,
            "round_id": "0",
            ...
            "source_ids": ["ctgov-ad-page-1"] if discovered else [],
    ...
    def _empty_round_receipt(dim: str, round_id: str) -> dict:
        ...
        receipt["receipt_id"] = f"ad-{dim}-r{round_id}"
    ...
    routes = [
        {
            ...
            "attempt_ids": ["attempt-global-1"],
            "source_ids": ["ctgov-ad-page-1", "ctgov-ad-page-2"],
        },
        {
            ...
            "source_ids": [],
            "query_or_identifier": "CDE/中国药物临床试验登记平台 AD",
            "diagnostic": (
    ...
    closure = {
        ...
        "alias_expansion_receipts": ["ad-alias-1", "ad-alias-r2"],
        "target_expansion_receipts": ["ad-target-1", "ad-target-r2"],
        "company_expansion_receipts": ["ad-company-1", "ad-company-r1"],
        "trial_expansion_receipts": ["ad-trial-1", "ad-trial-r1"],
        "independent_review_id": "ad-universe-review-1",
        "independent_reviewer_id": "independent-reviewer-ad",
        "independent_context": "clean-context-ad-vertical",
    ...
    def _fact(fid: str, row_ref: str, entity_id: str, name: str, field: str, value) -> dict:
        return {
            ...
            "source_id": (
                f"ctgov-ad-page-{nct_page.get(str(entity_id).lower(), 1)}"
                if str(entity_id).lower() in nct_page
                else next(
                    (
                        f"ctgov-ad-page-{nct_page.get(str(row.get('trial_id') or row.get('entity_id') or '').lower(), 1)}"
    ...
    report_payload["scientific_review"] = {
        "reviewer_id": "independent-reviewer-ad-m",
        "reviewer_role": "independent_scientific_verifier",
        "status": "accepted",
        "reviewed_at": "2026-09-20T03:30:00+00:00",
    ...
    audit = {
        "schema_version": "1.3",
        "package_id": "ad-vertical-audit-001",
        "contract_identity": {
        ...
        "producer_context": "producer-context-ad-vertical",
        "entities": entities,
    ...
    audit_path = HERE / "ad-audit-package.json"
