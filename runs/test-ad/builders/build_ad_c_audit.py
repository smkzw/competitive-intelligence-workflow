...
TARGET_TRIALS = (
    "NCT05131477", "NCT01945086", "NCT03345914",
    "NCT03912259", "NCT05034822", "NCT03920852",
)
COMPARATIVE_TRIALS = {"NCT05131477", "NCT01945086", "NCT03345914", "NCT03912259", "NCT05034822"}
...
def _row(trial_id: str, product_id: str, nct: str, page: int, family: str,
    ...
    row = {
        ...
        "randomization": None,
        "blinding": None,
        "source_version_id": f"ctgov-ad-page-{page}",
...
def _endpoint_form(measure: str) -> str:
    ...
    if "iga" in folded:
        return "IGA"
    if "pruritus" in folded or "itch" in folded or "nrs" in folded:
        return "PP-NRS"
    if "bsa" in folded:
        return "BSA"
    if "scorad" in folded:
...
def main() -> None:
    ...
    cas_root = PROJECT / "evidence/raw/sha256"
    ...
    for signature, ncts in sorted(groups.items()):
        ...
        candidate_paths.append({
            ...
            "summary_zh": (
                ...
                f"{'、'.join(endpoint_terms)} 等皮损与生活质量指标。"
            ),
            "assumptions_zh": (
    ...
    patterns = [
        _fact(
            "fact-pattern-dermatology",
            "六项登记试验的主要终点均围绕 EASI、IGA 等皮损改善与严重程度评分指标展开。",
            endpoint_rows,
        ),
        _fact(
            "fact-pattern-comparative",
            "多数试验采用随机双盲对照设计，设置安慰剂或溶剂对照组。",
            [grouping_rows[nct.lower()] for nct in TARGET_TRIALS
             if nct in COMPARATIVE_TRIALS],
        ),
    ]
    differences = [
        _fact(
            "fact-diff-control-strategy",
            "NCT03920852 为开放标签最大使用量研究；其余试验为平行双盲对照设计。",
            [grouping_rows[nct.lower()] for nct in TARGET_TRIALS],
        ),
        _fact(
            "fact-diff-phase-span",
            "试验阶段覆盖 I 期至 III 期；III 期试验以 dupilumab 等生物制剂系统治疗为主。",
            [row for row in observations if row["field"] == "trial_identity"],
        ),
    ]
    outliers = [
        _fact(
            "fact-outlier-pediatric",
            "NCT05034822 为儿童受试者人群研究，入组样本量较成人试验显著偏小。",
            [row for row in sample_rows if row["trial_id"] == "nct05034822"],
        ),
    ]
    ...
    content_payload = {
        ...
        "claims": [
            {
                "claim_id": f"claim-ad-c-page-{page_id}",
                "claim_text": (
                ...
                "fact_ids": [
                    ...
                    if row["source_version_id"] == f"ctgov-ad-page-{page_id}"
                ],
            }
    ...
    payload_out = {
        **json.loads(content.model_dump_json()),
        "scientific_review": {
            ...
            "status": "accepted",
            "reviewed_at": "2026-09-20T09:30:00+00:00",
