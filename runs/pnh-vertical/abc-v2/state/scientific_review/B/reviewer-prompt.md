# 独立科学复核任务（报告 B · 项目 abc-v1 · PNH 竞品调研工作流）
...
## 必读材料
1. `state/scientific_review/B/review_request.json` — 复核请求（含全部绑定字段）
2. `reports/B/v1/html/` — 已渲染门户（overview.html 等，可用浏览器或直接读 HTML）
3. `evidence/library/{r}-research-package.json`（r = a/b/c，对应你的报告类型）— 科学载荷：事实、来源、声明、独立复核签发
4. `reports/B/v1/html.manifest.json` — 渲染清单
...
## 产出（必须）
写文件 `state/scientific_review/B/verdict.json`，UTF-8 JSON，结构如下：
- `schema_version`: "1.0"
...
- `verdict_id`: "verdict-omp-deepseek-abc2-b"
