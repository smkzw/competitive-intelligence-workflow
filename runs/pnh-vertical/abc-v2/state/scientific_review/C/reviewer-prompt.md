# 独立科学复核任务（报告 C · 项目 abc-v1 · PNH 竞品调研工作流）
...
## 必读材料
1. `state/scientific_review/C/review_request.json` — 复核请求（含全部绑定字段）
2. `reports/C/v1/html/` — 已渲染门户（overview.html 等，可用浏览器或直接读 HTML）
3. `evidence/library/{r}-research-package.json`（r = a/b/c，对应你的报告类型）— 科学载荷：事实、来源、声明、独立复核签发
4. `reports/C/v1/html.manifest.json` — 渲染清单
...
## 产出（必须）
写文件 `state/scientific_review/C/verdict.json`，UTF-8 JSON，结构如下：
- `schema_version`: "1.0"
...
- `verdict_id`: "verdict-omp-mtplx-abc2-c"
- `verdict`: "accepted" 或 "veto"（依据你的真实复核结论）
- 如 veto：`veto_disposition` = "recoverable" 或 "exhausted"
- `reviewer_id`: "omp-mtplx-qwen38flash-xhigh"
- `reviewed_at`: 当前 UTC 时间（ISO-8601）
- `valid_until`: reviewed_at + 30 天

## 约束
- verdict.json 只能包含上方列出的字段（含可选 veto_disposition），禁止添加任何其他字段（如 verdict_summary、issues、verified_checks 等）——schema 严格拒绝额外属性
