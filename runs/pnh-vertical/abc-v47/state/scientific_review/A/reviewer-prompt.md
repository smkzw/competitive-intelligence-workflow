...
- `verdict_id`: "verdict-omp-gemini-abc47-a-r11"
...
## 严格 schema 禁令（违反将导致回执签发失败）
- verdict.json 顶层字段**仅限**上述列出的字段，不得添加 `verdict_summary`、`verified_checks`、`notes`、`observations`、`summary`、`details` 等任何其他字段
- issues 条目字段仅限 `issue_id`/`severity`/`category`/`description_zh`/`source_version_id`/`fragment_ids`，不得添加其他键
- 若结论为 accepted，整个文件不得包含 `issues` 与 `veto_disposition` 键
- issue 的 `fragment_ids` 只能从其 `source_version_id` 对应那条 source_refs 条目的 `fragment_ids` 列表逐字复制
...
- 完成后写规定的 verdict.json 并正常退出（退出码 0）。
