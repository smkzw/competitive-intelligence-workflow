...
(function () {
    ...
    return withMeta(
      {
        ...
        xAxis: {
          type: "category",
          data: categories,
          axisLabel: { interval: 0, hideOverlap: true, fontSize: 16 }
        },
        yAxis: {
          type: "value",
          name: unitLabel,
  ...
  function buildHeatmapOption(group) {
    ...
    for (var i = 0; i < group.rows.length; i++) {
      ...
      if (!isRenderable(row)) {
        ...
        seriesData.push({ value: [armIndex, eventIndex, num], rowIndex: i, status: null });
      }
    }
    return withMeta(
      {
    ...
    for (var i = 0; i < group.rows.length; i++) {
      ...
      if (!isRenderable(row)) {
        ...
        seriesData.push({
          ...
          name: row.trial_zh || row.product_zh || "试验",
          status: null,
    ...
    return withMeta(
      {
        tooltip: {
          trigger: "item",
          formatter: function (p) {
            return p.name + "<br>疗效差：" + p.value[0] + " 个百分点<br>不良事件：" + p.value[1] + "%<br>治疗组：" + p.value[2] + " 人";
          }
        },
  ...
  function positionBarEvidenceTargets(chartDiv, inst) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      targets[i].style.left = String(point[0] - 16) + "px";
      targets[i].style.top = String(point[1] - 16) + "px";
    }
  }
  ...
  function renderBarEvidenceTargets(chartDiv, inst, groupIndex, group) {
    ...
    for (var i = 0; i < group.rows.length; i++) {
      ...
      button.setAttribute("data-chart-index", String(chartIndex));
