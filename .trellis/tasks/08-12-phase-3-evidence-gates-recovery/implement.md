...
- [x] 创建 `gates/{exhaustion,blocker_audit}.py`、`blocker-audit.schema.json` 和四份指定集成测试。
- [x] 自动检索执行者完成多路径恢复，独立遗漏复核者逐缺口复审；角色、结论与回执分离，不能由同一自报替代。
- [x] 每个阻断报告只生成 `blockers/<report>/<version>/{audit.json,audit.md}`；中文说明列出医学对象、缺失内容、已尝试范围、是否需要用户、最小动作、原文/附件链接、唯一投递目录和恢复节点，不展示搜索日志或内部状态词。
- [x] `test_no_draft_when_blocked.py` 两层参数化：A 空创新药宇宙、B 无达到最低结果门槛的适格试验、C 无达到设计核心门槛的适格试验；再从 A/B/C GateSpec 枚举每个适用关键单元，构造非空候选且只缺该单元。
- [x] 每个缺口 case 断言：无锁定报告快照、coverage/projection、format job/render queue、artifact record；对应报告版本目录不存在正式、draft 或占位门户。允许的用户产物只有证据不足说明，内部回执可存在。
- [x] A/B/C 各一个非空关键冲突复用全部无草稿负断言；冲突不得伪装为未公开或数值零。
- [x] A/B/C 各一个 GateSpec 通过但独立科学质控否决场景：可修复者回 `recovering`，已穷尽者进 `evidence_blocked`；均无下游报告产物。
- [x] 精确命令通过：98 项；A/B/C 关键单元数 A=12、B=11、C=8，三个空场景、三个冲突和三个科学质控否决全部覆盖。
- [x] 两位隔离验收者修复后均为 `PASS；P0=0；P1=0`；主会场复测 98/143/429 并重放全部关键攻击后接受 Task 3.2。

### Task 3.2 最终攻击闭合（2026-08-12）

- [x] Phase 2 `EvidenceGap` 精确绑定缺口身份、对象、状态、候选路线、完成策略与信息增益。
- [x] 每个适用路线恰一条摘要；回执、尝试次数、访问方式、最终结果类别均由实际回执派生。
- [x] 科学路线允许真实 `content_acquired` 但缺字段；技术访问阻断必须以封闭技术失败类别结束。
- [x] 空宇宙只有一个资格缺口，绑定检索范围、科学缺失状态、真实 GateSpec 和对应资格字段。
- [x] B/C 空宇宙必须绑定闭合快照；A 真正零产品保持 `snapshot=None`。
- [x] 公共入口重新验证 `model_copy`、计算字段、旧摘要与跨报告/跨快照调包；同参重放幂等、内容漂移拒绝覆盖。

## Task 3.3–3.7

- [x] 下载请求、manual-inbox 自动识别、规范命名、隔离、归档与幂等恢复；两位隔离验收者最终 `P0=0、P1=0`，精确 3 项、既有 Phase 3 回归 241 项、全库 432 项通过。
- [x] GT01–GT11 类型化控制图、非法迁移拒绝、节点合同与重放幂等；共享事件库原始图事件在归约前复核，最终独立验收 `P0=0、P1=0`，全库 443 项通过。
- [x] 报告/格式独立的部分交付、阻断与恢复编排；补齐 `running`/`awaiting_user` 同时交付与穷尽时直达部分交付受阻的 v1.2 状态表勘误，独立验收 P0/P1=0，全库 445 项通过。
- [x] EX01–EX02、FX01–FX06 真实项目/fixture 执行器、唯一 catalog、规范产物路径、当前运行清单绑定和可审计恢复；最终独立复验 P0/P1/P2=0，全库 458 项通过。
- [ ] **下一项：**SQ01–SQ04 独立科学质控接受/否决。
