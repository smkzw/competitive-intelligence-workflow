...
**本文件只约束站点式 HTML**（多页面 / 说明书站点 / 门户 / ≥2 互通页 + 全局导航）。  
其他产物 **NEVER** 从本文件取版式合同；先读 `ROUTER.md`。

### 何时加载本轨

- `站点`、`多页面`、`说明书站点`、`门户`、`导航站`
- ≥2 个互通页面或需要全局搜索 / sticky 导航

### 硬边界

- **NEVER** 套用 1280×720 幻灯片坐标或翻页 runtime。
- **NEVER** 用单文档滚动报告冒充站点（单文档 → `track_stream.md`）。
- 品牌 / 证据 / AI 边界继承 `core.md`。
...
## 0.3 生成前的最小输入与默认决策（site）

开始制作前，Agent MUST 明确：站点信息架构、页面清单、导航层级、源材料。若用户已提供充分材料，不重复追问：

- 默认形态：多页 HTML + sticky 顶栏 + 页级 footer；**无**固定幻灯片画布。
- 默认背景：白色；顶栏默认浅色 + 深色导航字 + 橙色身份条（见 `core.md` §0.8）。
- 默认字体：§6.1；正文 ≥16 px。
- 默认数据：`file://` 安全路径（`data/*.js` 全局注入，规避 CORS）。
- 默认审阅：真实浏览器多页跳转 + 搜索/导航可用。
